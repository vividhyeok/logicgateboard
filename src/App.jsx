...
function Menu({ mode, setMode, level, setLevel, onStart, records, soundOn, onToggleSound, onExport }) {
  ...
  return <motion.main className="menu-page" initial={{opacity:0}} animate={{opacity:1}}>
    <header className="menu-header"><div className="brand-lockup"><motion.div className="brand-tile" initial={{rotate:-12,scale:.85}} animate={{rotate:-5,scale:1}} transition={{type:'spring'}}>LG</motion.div><div><span>DIGITAL PLAYTEST TABLE</span><h1>LOGIC GATE <em>DUEL</em></h1><p>실물 제작 전에 카드의 손맛, 맵 흐름, 선·후공 밸런스를 직접 확인하세요.</p></div></div><div className="menu-tools"><button className="icon-text-button" onClick={onToggleSound}><Icon name={soundOn?'sound':'mute'}/> {soundOn?'SOUND ON':'SOUND OFF'}</button><button className="icon-text-button" onClick={onExport} disabled={!records.length}><Icon name="download"/> TEST LOG {records.length}</button></div></header>
    ...
    <footer className="menu-footnote"><span>20 GATE CARDS · 2 WILD · SECRET INPUT · 2 PLAYERS</span><span>PLAYTEST BUILD 2.0</span></footer>
  </motion.main>
}
