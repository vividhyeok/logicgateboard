...
function Menu({ mode, setMode, level, setLevel, onStart, records, soundOn, onToggleSound, onExport }) {
  ...
  return <motion.main className="menu-page" initial={{opacity:0}} animate={{opacity:1}}>
    <header className="menu-header"><div className="brand-lockup"><a className="brand-tile home-tile" href="/" aria-label="처음 화면">←</a><div><span>혼자 하기</span><h1>맵을 고르세요</h1><p>논리 게이트를 놓아 원하는 OUTPUT을 만드세요.</p></div></div><div className="menu-tools"><button className="icon-text-button" onClick={onToggleSound}><Icon name={soundOn?'sound':'mute'}/> {soundOn?'소리 켜짐':'소리 꺼짐'}</button></div></header>
    ...
    <footer className="menu-footnote"><span>카드를 선택하고 빛나는 빈칸에 놓으세요.</span></footer>
  </motion.main>
}
...
export default function App() {
  ...
  function runCircuit(){ if(!game||game.phase!=='reveal'||resolving||resolutionStarted.current)return; resolutionStarted.current=true; const resolved=resolveGame(game); setSolution(resolved); setResolving(true); setRevealIndex(-1); sound('flip'); resolved.revealOrder.forEach((nodeId,index)=>queueTimeout(()=>{ setRevealIndex(index); sound('signal',{value:resolved.signals[nodeId]}) },650+index*760)); queueTimeout(()=>{ const finished=finishGame(game,resolved); setGame(finished); setResolving(false); sound('win'); const record=recordFromGame(finished); setRecords((current)=>{ const next=[...current,record]; localStorage.setItem(STORAGE_KEY,JSON.stringify(next)); return next }) },1150+resolved.revealOrder.length*760) }
  ...
  const phaseText=useMemo(()=>{ if(!game)return''; if(game.phase==='target_choice')return'목표 선택'; if(game.phase==='input_selection')return'비밀 INPUT'; if(game.phase==='play')return game.currentPlayer===0?'내 차례':'상대 차례'; if(game.phase==='reveal')return'회로 계산 중'; return'게임 종료' },[game,resolving])
