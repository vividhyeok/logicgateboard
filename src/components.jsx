  ...
  return (
    <motion.section className="board-frame" initial={{ opacity: 0, scale: 0.94, rotateX: 9 }} animate={{ opacity: 1, scale: 1, rotateX: 0 }} transition={{ type: 'spring', stiffness: 170, damping: 22 }}>
      ...
      <div className="board-canvas" data-resolving={resolving ? 'true' : 'false'}>
        ...
        <div className="board-fold vertical"/><div className="board-fold horizontal"/><span className="board-edition">PLAYTEST EDITION · 2026</span>
      </div>
    </motion.section>
  )
}
...
export function CoinOverlay({ winnerId, onDone }) { return <motion.div className="coin-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><motion.div className={`coin player-${winnerId+1}`} initial={{rotateY:0,y:-80,scale:.5}} animate={{rotateY:1080,y:0,scale:1}} transition={{duration:1.05,ease:[.2,.7,.2,1]}}>P{winnerId+1}</motion.div><motion.div className="coin-copy" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:.72}}><span>COIN TOSS</span><strong>PLAYER {winnerId+1} 목표 선택</strong><small>반대 플레이어가 먼저 카드를 놓습니다.</small><button type="button" onClick={onDone}>계속</button></motion.div></motion.div> }
...
export function ResultOverlay({ game, map, onReplay, onMenu, onNewSeed, onSaveFeedback }) { const winner=game.result?.winner??0; const [rating,setRating]=React.useState(3); const [note,setNote]=React.useState(''); return <motion.div className="result-overlay" initial={{opacity:0}} animate={{opacity:1}}><motion.div className={`result-card player-${winner+1}`} initial={{y:45,scale:.9}} animate={{y:0,scale:1}} transition={{type:'spring',stiffness:230,damping:22}}><span className="eyebrow">CIRCUIT COMPLETE</span><div className="result-output"><span>OUTPUT</span><strong>{game.result?.output}</strong></div><h2>PLAYER {winner+1} WIN</h2><p>{map.code} · {map.name} · {game.moves.length} turns</p><div className="result-actions"><button onClick={onReplay}>같은 판 다시</button><button onClick={onNewSeed}>새 판</button><button onClick={onMenu}>맵 선택</button></div><details className="feedback-box"><summary>플레이테스트 메모 남기기</summary><div className="rating-row"><span>재미/템포</span>{[1,2,3,4,5].map((value)=><button key={value} className={rating===value?'active':''} onClick={()=>setRating(value)}>{value}</button>)}</div><textarea value={note} onChange={(event)=>setNote(event.target.value)} placeholder="후공이 유리했다, Wild가 너무 강했다 등"/><button className="save-feedback" onClick={()=>onSaveFeedback({rating,note})}>기록 저장</button></details></motion.div></motion.div> }
