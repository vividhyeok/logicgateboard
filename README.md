...
- 입력 준비는 0/1 카드 한 번 클릭 → 자동 배치 → 확정 순서로 진행합니다. 확정 전에는 다른 카드를 누르거나 보드의 내 입력을 눌러 값을 바꿀 수 있습니다. 드래그 배치도 지원합니다.
...
- 혼자 하기와 온라인의 입력 준비 화면을 같은 보드·손패 컴포넌트로 통일했습니다.
- 낮은 가로 화면은 오른쪽 손패, 태블릿 세로 화면은 아래쪽 손패로 배치합니다. 700px 미만 화면에서는 보드를 가로 스크롤할 수 있습니다.
- 코인 연출은 2초 뒤 자동 진행하며 바로 넘기기도 가능합니다. 와일드 뒤집기는 해당 면 선택까지 한 번에 처리합니다. Esc로 카드 선택을 취소할 수 있습니다.
- 카드가 손에서 보드로 이동하고, 마지막 배치 자리를 표시하며, 결과 신호는 이미 공개된 경로를 다시 재생하지 않고 순서대로 흐릅니다. 시스템의 동작 줄이기 설정을 존중합니다.
- 결과 보기·보드 확인·새 판을 React 상태로 처리합니다. DOM을 직접 삭제하거나 공유 버튼 클릭을 가로채던 별도 보정 코드를 제거했습니다.
- 게임 방법, 키보드 포커스, 소리 설정을 일관되게 제공하며 저장 공간이 막혀도 혼자 하기는 계속됩니다.
- 온라인의 잘못된 드롭은 전송하지 않고, 거절된 명령은 현재 상태로 복구합니다. 연결 복구 중에는 진행을 잠시 멈춥니다.
...
브라우저 테스트는 1366×768, 1024×768, 768×1024에서 6개 맵의 입력 선택·교체, 게임 완료, 결과 확인, 재시작을 검사합니다. 드래그, 키보드, 저장 공간 접근 실패, 동작 줄이기도 포함합니다. Chromium과 WebKit에서 실제 터치 이벤트로 입력·카드 배치와 화면 회전을 별도로 검사합니다.

실제 Firebase 두 클라이언트 검증은 별도 실행합니다. 설정된 DB에 임시 방을 만들고 끝나면 해당 방을 닫습니다. 기본 테스트에서는 외부 DB를 변경하지 않습니다.

```powershell
$env:RUN_LIVE_ONLINE='1'
npx playwright test tests/browser/online.spec.js
```

실물 iPad/Android 기기의 브라우저·터치·전체 화면 지원 차이는 출시 기기에서 추가 확인하는 것이 좋습니다. 자동 검증은 브라우저의 화면 크기 및 터치 에뮬레이션을 사용합니다.
